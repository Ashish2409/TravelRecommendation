...
import com.epam.recommendation_service.repository.RatingRepository;
...
public class BlogServiceImpl implements BlogService{
    ...
    BlogSummaryResponse mapToBlogSummaryDTO(Blog blog) {
        ...
        blogSummaryResponse.setRatings(calculateAverageRating(blog.getId()));
    ...
    public List<BlogSummaryResponse> getMyBlogs(String email) {
            ...
            return blogs.stream()
                    .map(blog -> new BlogSummaryResponse(
                            ...
                            calculateAverageRating(blog.getId()),0
                    ))
                    .collect(Collectors.toList());
    ...
    @Override
    public void addOrUpdateRating(Integer blogId, String userEmail, int ratingValue) {
        Optional<Blog> blogOptional = blogRepository.findById(blogId);
        if (blogOptional.isPresent()) {
            Blog blog = blogOptional.get();
            Optional<Rating> existingRating = blog.getRatings().stream()
                    .filter(r -> r.getUserEmail().equals(userEmail))
                    .findFirst();

            Rating rating = existingRating.orElseGet(() -> {
                Rating newRating = new Rating(userEmail, blog, ratingValue);
                blog.getRatings().add(newRating);
                return newRating;
            });
    ...
    @Override
    public double calculateAverageRating(Integer blogId) {
        Optional<Blog> blogOptional = blogRepository.findById(blogId);
        if (!blogOptional.isPresent()) {
            throw new BlogNotFoundException("Blog not found with ID: " + blogId);
        }
        try {
            List<Rating> ratings = ratingRepository.findByBlogId(blogId);
            ...
            throw new BlogNotFoundException("Blog not found while retriving ratings...");
