...
class BlogControllerTest {
    ...
    void testFilterBlogsByCountry() {
        Blog blog1 = new Blog(1, "Blog 1", "Description 1", null, "Category 1", "City 1", "Country 1", "email1@example.com", LocalDateTime.now(), "Author 1", "PENDING",new ArrayList<>(),new ArrayList<>());
        Blog blog2 = new Blog(2, "Blog 2", "Description 2", null, "Category 2", "City 2", "Country 2", "email2@example.com", LocalDateTime.now(), "Author 2", "PUBLISHED",new ArrayList<>(),new ArrayList<>());
        Blog blog3 = new Blog(3, "Blog 3", "Description 3", null, "Category 3", "City 3", "Country 2", "email3@example.com", LocalDateTime.now(), "Author 3", "PENDING",new ArrayList<>(),new ArrayList<>());
        List<Blog> filteredBlogs = Arrays.asList(blog2, blog3);
    ...
    public void testAddOrUpdateRating() {
        ...
        doNothing().when(blogService).addOrUpdateRating(blogId, userEmail, ratingValue);

        ResponseEntity<?> response = blogController.addOrUpdateRating(blogId, userEmail, ratingValue);
        assertEquals(HttpStatus.OK, response.getStatusCode());
    }
    ...
    public void testCalculateAverageRating() {
        ...
        when(blogService.calculateAverageRating(blogId)).thenReturn(averageRating);

        ResponseEntity<Double> response = blogController.calculateAverageRating(blogId);
        assertEquals(HttpStatus.OK, response.getStatusCode());
        assertEquals(averageRating, response.getBody());
    }

    @Test
    public void testAddOrUpdateRatingWithInvalidBlogId() {
        int blogId = -1;
        String userEmail = "user@example.com";
        int ratingValue = 4;

        doNothing().when(blogService).addOrUpdateRating(anyInt(), anyString(), anyInt());
    ...
    @Test
    public void testCalculateAverageRatingWithInvalidBlogId() {
        int blogId = -1;

        when(blogService.calculateAverageRating(blogId)).thenReturn(0.0);
