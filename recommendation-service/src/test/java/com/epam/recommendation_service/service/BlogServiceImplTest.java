...
import com.epam.recommendation_service.repository.*;
...
class BlogServiceImplTest {
    ...
    void setUp() {
        ...
        Blog blog1 = new Blog(1, "Blog 1", "Description 1", null, "Category 1", "City 1", "Country 1", "email1@example.com", LocalDateTime.now(), "Author 1", "PENDING",new ArrayList<>(),new ArrayList<>());
        Blog blog2 = new Blog(2, "Blog 2", "Description 2", null, "Category 2", "City 2", "Country 2", "email2@example.com", LocalDateTime.now(), "Author 2", "PUBLISHED",new ArrayList<>(),new ArrayList<>());
        Blog blog3 = new Blog(3, "Blog 3", "Description 3", null, "Category 2", "City 3", "Country 3", "email3@example.com", LocalDateTime.now(), "Author 3", "PENDING",new ArrayList<>(),new ArrayList<>());
        Blog blog4 = new Blog(4, "Blog 4", "Description 4", null, "Category 3", "City 4", "Country 2", "email4@example.com", LocalDateTime.now(), "Author 4", "PUBLISHED",new ArrayList<>(),new ArrayList<>());
    ...
    void getBlogsByPage_Success() {
        ...
        when(blogRepository.findById(1)).thenReturn(Optional.of(blog)); // Mock blogRepository.findById(1)
        ...
        when(ratingRepository.findByBlogId(1)).thenReturn(new ArrayList<>()); // Mock ratingRepository.findByBlogId(1)
    ...
    void getAllFavouriteBlogs_Success() {
        ...
        when(blogRepository.findById(1)).thenReturn(Optional.of(blog1));
        when(blogRepository.findById(2)).thenReturn(Optional.of(blog2));
        when(ratingRepository.findByBlogId(1)).thenReturn(new ArrayList<>());
        when(ratingRepository.findByBlogId(2)).thenReturn(new ArrayList<>());
    ...
    @Test
    public void testAddOrUpdateRatingForExistingBlog() {
        int blogId = 1;
        ...
        Blog blog = new Blog();
        blog.setId(blogId);
        Rating existingRating = new Rating(userEmail, blog, 3);
        blog.getRatings().add(existingRating);

        when(blogRepository.findById(blogId)).thenReturn(Optional.of(blog));

        blogService.addOrUpdateRating(blogId, userEmail, ratingValue);

        assertEquals(ratingValue, existingRating.getRating());
    }

    @Test
    public void testAddOrUpdateRatingForNewRating() {
        int blogId = 1;
        String userEmail = "user@example.com";
        int ratingValue = 4;
        ...
        assertEquals(1, blog.getRatings().size());
        Rating rating = blog.getRatings().get(0);
        assertEquals(userEmail, rating.getUserEmail());
        assertEquals(ratingValue, rating.getRating());
    }

    @Test
    public void testAddOrUpdateRatingForNonExistingBlog() {
        int blogId = 1;
        String userEmail = "user@example.com";
        int ratingValue = 4;
        ...
        BlogNotFoundException exception = assertThrows(BlogNotFoundException.class, () -> blogService.addOrUpdateRating(blogId, userEmail, ratingValue));
        assertEquals("Blog not found with ID: " + blogId, exception.getMessage());
    }

    @Test
    public void testCalculateAverageRatingForExistingBlog() {
        int blogId = 1;

        Blog blog = new Blog();
        blog.setId(blogId);
        Rating rating1 = new Rating("user1@example.com", blog, 3);
        Rating rating2 = new Rating("user2@example.com", blog, 4);
        blog.getRatings().addAll(Arrays.asList(rating1, rating2));

        when(blogRepository.findById(blogId)).thenReturn(Optional.of(blog));
        when(ratingRepository.findByBlogId(blogId)).thenReturn(blog.getRatings());

        double averageRating = blogService.calculateAverageRating(blogId);

        assertEquals(3.5, averageRating);
    }

    @Test
    public void testCalculateAverageRatingForNonExistingBlog() {
        int blogId = 1;

        when(blogRepository.findById(blogId)).thenReturn(Optional.empty());

        assertThrows(BlogNotFoundException.class, () -> blogService.calculateAverageRating(blogId));
    }

    @Test
    public void testCalculateAverageRatingForBlogWithNoRatings() {
        int blogId = 1;
        ...
        when(blogRepository.findById(blogId)).thenReturn(Optional.of(blog));
        when(ratingRepository.findByBlogId(blogId)).thenReturn(List.of());
